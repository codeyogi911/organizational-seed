...
**Required frontmatter:** `id`, `kind`, `date`, `source-process`, `applies-to`,
and `status`. `kind` is **lesson**; `source-process` names the Process whose
...
found one. `status` is **pending**, **absorbed**, or **retired**.
...
- **Reroute:** change `applies-to:` to the correct proposed Standing Knowledge
  home, or to `unresolved`, and keep `status: pending`.
