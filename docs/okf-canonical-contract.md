...
After canonical migration, the same profile declares `source.root` as
`knowledge` and paths relative to that root. Recompiling to an external bundle
must reproduce the canonical values and bodies without an extra directory
prefix.
